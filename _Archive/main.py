...
def download_pdf(link, output_folder, filename):
    ...
    try:
        response = requests.get(link, stream=True)  # Stream the response for large files
...
def main():
    ...
    for property in property_data:
        ...
        pdf_filename = property.get("Matched Name", "Unknown").replace("/", "_")
        download_pdf(property.get("Link"), PDF_FOLDER, pdf_filename)
    ...
    # Completion message (appears only once now)
