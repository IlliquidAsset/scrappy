...
def confirm_match(input_name, matched_name, confirmed_matches, threshold=80):
    ...
    if partial_ratio(normalized_input, normalized_matched) >= threshold:
        from main import ask_confirmation  # Import here to avoid circular imports
        return ask_confirmation(matched_name, input_name)  # Swapped order to match the API's expectation
